...
class TaskIdentifierLibrary:
    ...
    def get_create_task_instances_query(resource):
        ...
        query_str = '''
                    :auto
                    CALL {
                          MATCH (e1:Event)-[:DF_JOINT]->()
                          WHERE NOT ()-[:DF_JOINT]->(e1)
                          MATCH ()-[:DF_JOINT]->(e2:Event)
                          WHERE NOT (e2)-[:DF_JOINT]->()
                          MATCH p = (e1)-[:DF_JOINT * ]->(e2)
                          RETURN p, e1, e2
                          UNION
                          MATCH (e:Event)
                          WHERE (e)-[:CORR]->(:$resource_node_label)
                          AND NOT ()-[:DF_JOINT]->(e) AND NOT (e)-[:DF_JOINT]->()
                          MATCH p = (e)
                          RETURN p, e AS e1, e AS e2
                    }
                    WITH [event IN nodes(p) | event.activity + '+' + event.lifecycle] AS variant,
                    nodes(p) AS events, e1.timestamp AS start_time, e2.timestamp AS end_time
                    CALL (variant, events, start_time, end_time) {
    ...
    def get_split_ti_nodes_create_new_1_query():
        ...
        query_str = '''
                    :auto
                    MATCH (ti:TaskInstance)-[:CONTAINS]->(e:Event)
                    WHERE date(ti.start_time) <> date(ti.end_time)
                    WITH ti, date(e.timestamp) AS date, e ORDER BY e.timestamp
                    WITH DISTINCT ti, date, COLLECT(e) AS events
                    WITH events[0] AS e_start, events[- 1] AS e_end
                    WITH e_start, e_end
    ...
    def get_aggregate_task_instances_query(property):
        ...
        query_str = '''
                    :auto
                    MATCH (ti:TaskInstance)
                    WHERE ti.$property IS NOT NULL
                    WITH DISTINCT ti.$property AS id, count( * ) AS COUNT
                    CALL (id, COUNT) {
    ...
    def get_link_task_instances_to_aggregations_query(property):
        ...
        query_str = '''
                    :auto
                    MATCH (ta:TaskAggregation)
                    WHERE ta.Type = '$property'
                    MATCH (ti:TaskInstance)
                    WHERE ti.$property = ta.id
                    WITH ta, ti
                    CALL (ta, ti) {
