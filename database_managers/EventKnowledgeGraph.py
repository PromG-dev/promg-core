...
class EventKnowledgeGraph:
    ...
    :param batch_size: the batch_size used when calling apoc.periodic.iterate --> reduce required memory, defaults to 5000
    :type batch_size: int
    :param use_sample: boolean indicating whether the DB should be build using a sample as specified in the ImportedDataStructures, defaults to False
    :type use_sample: bool

    """
    def __init__(self, db_connection: DatabaseConnection, db_name: str, specification_of_data_structures: ImportedDataStructures,
                 semantic_header: SemanticHeader, perf: Performance,
        ...
        self.data_importer = Importer(db_connection, data_structures=specification_of_data_structures, batch_size=batch_size,
    ...
    def get_event_log(self, entity_type:str, additional_event_attributes: Optional[List[str]]):
        """
        Pass on ekg_management to data_imported to get an event log extracted from the EKG for a specific entity and return it

        :param entity_type: The type of the entity --> contract states that it exists
        :type entity_type: str
        :param additional_event_attributes: list of different attributes of event that should also be stored in the event log
    ...
    def create_entities_by_nodes(self, entity_types: Optional[List[str]] = None) -> None:
        """
        Pass on method to ekg_builder to create relations between entities based on nodes as specified in the semantic header

        :param entity_types: list of entity types that should be created based on nodes. In case of None, all entities based on nodes are created as specified in the semantic header
    ...
    def correlate_events_to_entities(self, entity_types: Optional[List[str]] = None) -> None:
        ...
        :param entity_types: list of entity types that should be correlated to events. In case of None, all relations based on entities are correlated to events as specified in the semantic header
        :type entity_types: List[str], optional
        :return: None
    ...
    def create_entity_relations_using_nodes(self, relation_types: Optional[List[str]] = None) -> None:
        ...
        :param relation_types: list of relation types for which the relations should be created. In case of None, all relations based on relations are created as specified in the semantic header
    ...
    def create_entity_relations_using_relations(self, relation_types: Optional[List[str]] = None) -> None:
        """
        Pass on method to ekg_builder to create relations between entities based on relations as specified in the semantic header

        :param relation_types: list of relation types for which the relations should be created. In case of None, all relations based on relations are created for all relation types as specified in the semantic header
        :type relation_types: List[str], optional
        :return: None
    ...
    def create_entities_by_relations(self, entity_types: Optional[List[str]] = None) -> None:
        ...
        :param entity_types: list of entity types for which the entities based on relations should be created. In case of None, all entities based on relations are created as specified in the semantic header
        :type entity_types: List[str], optional
        :return: None
    ...
    def correlate_events_to_reification(self) -> None:
        ...
        :param entity_types: list of entity types for which the DFs should be created. In case of None, DFs are created for all EntityTypes as specified in the semantic header
        :type entity_types: List[str], optional
        :return: None
    ...
    def infer_items_propagate_upwards_multiple_levels(self, entity_type: str, is_load=True) -> None:
        ...
        :param entity_type: The type of the entity
        :type entity_type: str
        :param is_load: indicating whether we are inferring upwards to load events (true) or unload events (false), defaults to True
        :type is_load: bool
        :return: None
    ...
    def create_df_process_model(self, entity_type: str) -> None:
        """
